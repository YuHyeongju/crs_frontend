...
const HomePage = () => {
    ...
    const [showCongestionModal, setShowCongestionModal] = useState(false);
    const [selectedRestaurant, setSelectedRestaurant] = useState(null);
    const [isApiLoaded, setIsApiLoaded] = useState(false); // API 로딩 상태 추가
    const [initialLocationSet, setInitialLocationSet] = useState(false);
    ...
    // **핵심 수정 부분**: kakao.maps.load()를 사용하여 API 로딩 완료를 보장
    useEffect(() => {
        // 이미 API가 로드되었다면 다시 실행하지 않음
        if (isApiLoaded) return;
        
        // window.kakao.maps.load()가 존재할 때만 실행
    ...
    const startBlinkingUserMarker = useCallback(() => {
        if (!kakao || !userLocationMarkerRef.current || !mapInstance) return;
        stopBlinkingUserMarker();
        let isVisible = true;
        userLocationBlinkIntervalRef.current = setInterval(() => {
            if (userLocationMarkerRef.current) {
    ...
    }, [mapInstance, stopBlinkingUserMarker, kakao]);
    ...
    const createAndDisplayMarker = useCallback((place, map, index = null, onMarkerClick) => {
        ...
        kakao.maps.event.addListener(marker, 'click', () => {
            ...
            if (infoWindowRef.current) {
                kakao.maps.event.addListener(infoWindowRef.current, 'close', () => {
                    if (userLocationMarkerRef.current && userLocationBlinkIntervalRef.current === null) {
                        startBlinkingUserMarker();
                    }
                });
        ...
        return marker;
    }, [infoWindowRef, stopBlinkingUserMarker, startBlinkingUserMarker, userLocationMarkerRef, kakao]);

    const searchAndDisplayRestaurants = useCallback((centerLatLng, searchType = 'initial', keyword = '') => {
        if (!mapInstance || !kakao || !kakao.maps.services) {
            return;
        }
        removeRestaurantMarkers();
        setRestaurantList([]);
        ...
        if (searchType !== 'keyword') {
            ...
            searchOptions.radius = isMobile ? 500 : 20000;
        }
        ...
        const callback = (data, status) => {
            if (status === kakao.maps.services.Status.OK) {
                ...
                const newRestaurantList = [];
                data.forEach((place, index) => {
                    if (place.category_group_code === 'FD6' || searchType === 'keyword') {
                        const additionalData = generateDynamicDetails();
                        const mergedPlace = { ...place, ...additionalData };

                        const marker = createAndDisplayMarker(mergedPlace, mapInstance, index + 1, handleMarkerClick);
                        newMarkers.push(marker);
                        bounds.extend(new kakao.maps.LatLng(mergedPlace.y, mergedPlace.x));
                        newRestaurantList.push(mergedPlace);
                    }
                });
                restaurantMarkersRef.current = newMarkers;
    ...
    }, [mapInstance, removeRestaurantMarkers, createAndDisplayMarker, isMobile, handleMarkerClick, kakao]);
    ...
    const handleKeywordSearch = useCallback(() => {
        ...
        searchAndDisplayRestaurants(null, 'keyword', searchTerm);
        setShowRestaurantPanel(true);
    }, [mapInstance, searchTerm, searchAndDisplayRestaurants, stopBlinkingUserMarker]);
    ...
    useEffect(() => {
        if (mapContainerRef.current && isApiLoaded && !initialLocationSet) {
            const mapContainer = mapContainerRef.current;
            const mapOption = {
            ...
            `;
            const redMarkerDataUrl = `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(redCircleSvg)}`;
            userMarkerImageRef.current = new kakao.maps.MarkerImage(
                redMarkerDataUrl,
    ...
    }, [isApiLoaded, currentUserCoords, initialLocationSet, kakao, startBlinkingUserMarker, searchAndDisplayRestaurants]);
    ...
    useEffect(() => {
        ...
        if (mapInstance && kakao) {
            clickHandler = function (mouseEvent) {
                ...
                const newClickedMarker = new kakao.maps.Marker({
                    position: latlng,
                    map: mapInstance
                });
                clickedMarkerRef.current = newClickedMarker;
            ...
            kakao.maps.event.addListener(mapInstance, 'click', clickHandler);
        }
        return () => {
            if (mapInstance && clickHandler && kakao) {
                kakao.maps.event.removeListener(mapInstance, 'click', clickHandler);
            }
        };
    }, [mapInstance, searchAndDisplayRestaurants, stopBlinkingUserMarker, kakao]);
    ...
    const handleGoToMyLocation = useCallback(() => {
        ...
        if (mapInstance && userMarkerImageRef.current && kakao) {
            const { latitude, longitude } = currentUserCoords;
            const userLatLng = new kakao.maps.LatLng(latitude, longitude);
            if (clickedMarkerRef.current) {
                clickedMarkerRef.current.setMap(null);
    ...
    }, [mapInstance, currentUserCoords, searchAndDisplayRestaurants, startBlinkingUserMarker, kakao]);
