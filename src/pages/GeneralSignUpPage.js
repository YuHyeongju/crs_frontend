...
function GeneralSignUpPage() {
    ...
    const [phoneError, setPhoneError] = useState('');
    const [emailError, setEmailError] = useState(''); // 이메일 유효성 메시지
    const [formError, setFormError] = useState(''); // 폼 제출 시 전체 오류 메시지
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setFormError(''); // 제출 시 기존 오류 메시지 초기화
        // setSuccessMessage(''); // ⭐ 삭제: 제출 시 성공 메시지 초기화

        // 필수 필드 검사
        if (!formData.userId || !formData.password || !formData.confirmPassword || !formData.name || !formData.email || !formData.phone || !formData.gender) {
            setFormError('모든 필수 필드를 채워주세요.');
            return;
        }
        ...
        if (phoneError || formData.phone.length < 10) {
            setFormError('전화번호를 올바르게 입력해주세요 (숫자만 입력, 최소 10자리).');
            return;
        }

        if (emailError || formData.email === '') {
            setFormError('이메일을 올바른 형식으로 입력해주세요.');
            return;
        }
...
const styles = {
    ...
    submitButtonHover: { // 현재 사용되지 않음 (인라인 스타일에서 직접 처리)
        backgroundColor: '#0056b3',
    },
    matchMessage: {
        color: 'green',
