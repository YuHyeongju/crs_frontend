// src/pages/MerchantSignUpPage.js
...
function MerchantSignUpPage() {
  ...
  const [businessNumError, setBusinessNumError] = useState(''); // 사업자 번호 유효성 메시지
  ...
  const handleChange = (e) => {
    ...
    // 사업자 등록 번호(businessNum) 처리 로직
    else if (name === 'businessNum') {
      const numericValue = value.replace(/[^0-9]/g, '');
      let newBusinessNumValue = numericValue;

      if (numericValue.length > BUSINESS_NUM_LENGTH) {
        newBusinessNumValue = numericValue.slice(0, BUSINESS_NUM_LENGTH);
        setBusinessNumError(`사업자 등록 번호는 ${BUSINESS_NUM_LENGTH}자리 숫자여야 합니다.`);
      } 
      else if (value !== numericValue) {
        setBusinessNumError('사업자 등록 번호는 숫자만 입력 가능합니다.');
      }
      else if (value.length > 0 && value.length < BUSINESS_NUM_LENGTH) {
        setBusinessNumError(`사업자 등록 번호는 ${BUSINESS_NUM_LENGTH}자리여야 합니다. (현재 ${value.length}자리)`);
      }
      else {
        setBusinessNumError('');
      }
      setFormData({ ...formData, [name]: newBusinessNumValue });
    }
    // 일반적인 입력 처리
    else {
      setFormData({ ...formData, [name]: value });
  ...
  const handleSubmit = (e) => {
    ...
    if (businessNumError || formData.businessNum.length !== BUSINESS_NUM_LENGTH || !/^[0-9]{10}$/.test(formData.businessNum)) {
      alert(`사업자 등록 번호는 ${BUSINESS_NUM_LENGTH}자리의 숫자로 구성되어야 합니다.`);
      return;
    }
    ...
    const payload = {
      ...
      BUSINESS_NUM: formData.businessNum,
    };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.signUpBox}>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.inputGroup}>
            <label htmlFor="businessNum" style={styles.label}>사업자 등록 번호</label>
            <input
              type="text"
              id="businessNum"
              name="businessNum"
            ...
            />
            {/* 사업자 등록 번호 에러 메시지 표시 */}
            {businessNumError && <p style={styles.phoneErrorMessage}>{businessNumError}</p>}
          </div>
