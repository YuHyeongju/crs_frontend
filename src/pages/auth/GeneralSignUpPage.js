...
function GeneralSignUpPage() {
    ...
    const [formData, setFormData] = useState({
        id: '', pw: '', confirmPw: '', name: '', email: '', phone: '', gender: '',
    });
    ...
    const handleChange = (e) => {
        ...
        if (name === 'pw') {
            ...
            const reg = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            setEmailError(value !== '' && !reg.test(value) ? '유효한 이메일 형식이 아닙니다.' : '');
            setFormData({ ...formData, [name]: value });
        } else {
            setFormData({ ...formData, [name]: value });
        }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.gender) { setFormError('성별을 선택해주세요.'); return; }
        if (pwError || phoneError || emailError || !passwordMatch) { setFormError('입력 형식을 확인해주세요.'); return; }
        try {
            const res = await axios.post("/api/auth/register/user", formData);
            if (res.status === 201 || res.status === 200) { alert('회원가입이 완료되었습니다!'); navigate("/login"); }
        } catch (err) { setFormError(err.response?.data || '서버 오류가 발생했습니다.'); }
    };
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.signUpBox}>
                <h2 style={styles.title}>일반 회원가입</h2>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <div style={styles.inputGroup}><label style={styles.label}>이메일</label><input type="email" name="email" value={formData.email} onChange={handleChange} required style={styles.input} />{emailError && <p style={styles.errorMessage}>{emailError}</p>}</div>
